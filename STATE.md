...
## Where things stand
- **Branch chain (all local, no remote):** `main` <- `feature/m1-engine` <- `feature/howclose-trial` <- `feature/regulate-kit` <- `feature/phase2-machine` (merged to main fce5a38) <- `feature/layout-lints` (merged to main 2fd01a5) <- **`feature/cta-lint`** (09-04 third foreman run: real CTA lint with 2500ms dwell, overlap lint, script --url, thought/figure geometry fixes). **One merge gets everything:** `git -C ~/Documents/brandreel merge --no-ff feature/cta-lint`. Live worktrees: /tmp/brandreel-reg-wt (reg-kit, provides engine/node_modules), /private/tmp/fm-brandreel-state (cta-lint). fm/* branches are merged and disposable.
- **Gates all green on cta-lint:** typecheck, 29 engine tests, 61 bin tests (`node --test bin/tests/`). Chain: `node bin/script.mjs workspace/<id> --brand regulate --topic "..."` (claude -p; retries against the manifest lints) -> `node bin/reel.mjs workspace/<id> --skip vo,align,polish` (manifest -> compose -> lint -> review). workspace/smoke-3am-v3 passes all 11 lint rules (script.mjs takes --url; the model never sets close.url); smoke-3am and smoke-3am-v2 are kept failing baselines. Audio venv not present in these worktrees (rebuild per audio/README.md).
