...
- **Lint coverage now:** ffprobe facts, safe zones + text fit from layout.json (glyph-width estimate), hook, pacing (static > 3s), CTA (literal SPEC reading; closeDwellMs reported, minimum dwell is gate G5), pixel bands on sampled frames. Not covered: caption vs Moment-thought overlap (same lower zone), real glyph measurement (estimate only).vo/music.
