...
3. **First real Regulate video = 3:04 AM** (animatic at gate, above). After KEEP: `exhale` beat kind (Codex via /dev-delegate), 1:1 port, compose -> lint -> review. Second video = The Sigh (workspace/regulate-sigh exists as a brief-driven text version; the orb-free breath-paced animatic is still to author).
...
- **Animatic verification:** the in-app Browser pane freezes CSS animations while hidden (document.visibilityState = hidden, every animation at currentTime 0) and Chrome's --virtual-time-budget stalls rAF players. Use `node bin/animatic-frames.mjs <file> <out-dir> <ms...>` - real-time headless Chrome via DevTools protocol; frames are named by actual capture time (~2s per frame).
