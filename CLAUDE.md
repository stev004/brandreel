...
- `brand.json` (Zod, `engine/src/schema.ts`) is the portability boundary. Templates never contain brand literals.
...
- Renders need Chromium: sandboxed implementers stop at `tsc --noEmit` + tests; reviewer or CI runs `bin/compose.mjs`.
