...
- [ ] **G1a** Add the `exhale` beat kind to the engine, 1:1 with `animatics/regulate-3am.html` (its header comment is the porting contract; `animatics/regulate-3am.plan.md` has the decisions).
  Files: `engine/src/schema.ts` (new beat kind), `engine/src/templates/Exhale.tsx` (new), `engine/src/layout.ts` (constants + computeTextBoxes + computeTimeline for the new kind), `engine/src/Stack.tsx` (dispatch), `engine/tests/*`. The `moment` beat gets optional per-thought `{x, y}` placement so S1 thoughts sit where the animatic puts them (absolute stage coordinates), because the exhale scene shares those positions.
