...
| G6 | The Regulate tagline "Not meditation. Regulation." (27 chars) cannot fit the CloseD tagline row (96px display, 18 chars). Shrink the tagline face, allow two rows, or use the url-only close for Regulate? | smaller face / two rows / url-only | untouched (taste); scripts pass --tagline only when it fits | open |
