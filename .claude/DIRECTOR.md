...
- 2026-09-05: the director logged "existing script still validates" from a one-off eval that had thrown a TypeError further up the output -> RULE: read the whole output of a verification command before writing its trail row; a row written from a partial read is superseded, never edited.
