...
9. **Craft laws (adopted 2026-09-02 from an audited external prompt-discipline doc; they apply to OUR authoring, not to any external tool):**
   - **One core mechanic per video.** Every script declares `coreMechanic`: one sentence naming the single conceptual engine a viewer could describe ("the honest short line vs the long dashed road"). Stage-1 authoring invents the mechanic FIRST, then writes beats that prove it; a script whose beats do not express its mechanic fails review. Lint: the field is required and non-empty; the human gate judges whether the beats deliver it.
   - **Copy list is a contract.** Every string that appears on screen exists in script.json; templates render only prop strings (already enforced by the no-literals guard). Nothing improvised at the template layer, ever.
   - **Style as constraints, not adjectives.** Every brand.json carries a `forbidden` list in voice.notes or a dedicated field - explicit prohibitions ("no springs, no gauges, no glow, no grain") that reviews check against, instead of taste adjectives. howclose and regulate kits already comply; every future kit must.
   - **Motion semantics (2026-09-02, from the fusion-video audits).** The easing and duration of every data animation must match what the data IS: a cumulative record only ratchets upward; an instantaneous event snaps (~200-300ms); a long struggle draws slowly and linearly; a comparison uses forms the viewer already reads (bars beat time-series for lay audiences). Audit each element with one question: what is this moment saying, and does the motion say it? Definitions persist across cuts instead of dying one scene before they are needed; labels appear where and when they mean something.
